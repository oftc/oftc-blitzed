...
#include <string.h>
#undef getline
unsigned int numstrings = 0;	/* Number of strings we should have */
...
static int fput32(int val, FILE *f);
...
int main(int ac, char **av)
{
    ...
    if ((size_t)(s-filename) > sizeof(langname)-3)
	s = filename + sizeof(langname)-1;
    strncpy(langname, filename, (size_t)(s-filename));
