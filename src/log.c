...
static int rename_log(User * u, const char *newname);
...
static int rename_log(User * u, const char *newname)
{
	...
	if ((temp_fp = fopen(newname, "r"))) {
		fclose(temp_fp);
		if (u) {
			notice_lang(s_OperServ, u, OPER_ROTATELOG_FILE_EXISTS,
				    newname);
		} else {
			wallops(NULL,
				"WARNING: Could not rename logfile, a file with "
				"the name \2%s\2 already exists!", newname);
		}
		log
		    ("ERROR: Could not rename logfile, a file with the name \2%s\2 "
		     "already exists!", newname);
		return 0;
	}

	if (debug)
		log("Renaming logfile from %s to %s ...", log_filename,
		    newname);
	...
	if (rename(log_filename, newname) == 0) {
		if (u) {
			notice_lang(s_OperServ, u, OPER_ROTATELOG_RENAME_DONE,
				    newname);
		}
		...
		if (u) {
			...
			wallops(NULL,
				"WARNING: Logfile could not be renamed: %s",
				strerror(errno));
		}
		success = 0;
	}
	...
	if (open_log() == -1) {
		if (u) {
			...
			wallops(NULL,
				"WARNING: Logging could not be restarted: \2%s\2",
				strerror(errno));
			wallops(NULL,
				"Please notify your services administrator, \2%s\2, "
				"and include the above error message.",
				ServicesRoot);
			success = 0;
		}
	}
	...
	if (success) {
		log
		    ("Fresh logfile started. Previous logfile has been renamed to: %s",
		     newname);
	} else if (debug) {
		/* pointless if open_log() fails */
		log("ERROR: Logfile could not be renamed: %s",
		    strerror(errno));
	}
...
void log_perror(const char *fmt, ...)
{
	...
	if (debug) {
		...
		s = buf + strlen(buf);
		s +=
		    snprintf(s, sizeof(buf) - (s - buf), ".%06ld", tv.tv_usec);
		strftime(s, sizeof(buf) - (s - buf) - 1, " %Y] ", &tm);
	} else {
#endif
		strftime(buf, sizeof(buf) - 1, "[%b %d %H:%M:%S %Y] ", &tm);
...
/* snoop: PRIVMSG our snoop channel as defined in services.conf
 *        as SnoopChan.  Based on skold's wrecked.net implementation.
 *        -grifferz
 */

void snoop(const char *source, const char *fmt, ...)
{
        if (SnoopChan && findchan(SnoopChan)) {
                va_list args;
		time_t t;
		struct tm tm;
                char buf[BUFSIZE], timebuf[64];

                va_start(args, fmt);
		time(&t);
		tm = *localtime(&t);
		strftime(timebuf, sizeof(buf) - 1, "[%b %d %H:%M:%S %Y] ", &tm);
                snprintf(buf, sizeof(buf), "PRIVMSG %s :%s %s", SnoopChan, timebuf, fmt);
...
void fatal_perror(const char *fmt, ...)
{
	...
	if (stderr)
		fprintf(stderr, "%sFATAL: %s: %s\n", buf, buf2,
			strerror(errno_save));
	if (servsock >= 0)
		wallops(NULL, "FATAL ERROR!  %s: %s", buf2,
			strerror(errno_save));
	exit(1);
}
